				...
				<table id="example11" class="table table-striped table-bordered table-hover" width="100%">
					<thead>
						<tr bgcolor='#9acfea'>
							...
							<th class="no-sort text-center">No Ref</th>
							<!--<th class="no-sort text-center" width="10px">Action</th>-->
						</tr>
					</thead>
					<tbody>
						<?php
...
/*
								...
								if ($data_uang_keluar_dari > 0) {
									foreach ($data_uang_keluar_dari as $row_jurnal1) {
										...
										$data_buk_coa	= $this->db->query("SELECT * FROM COA WHERE no_perkiraan = '$uang_keluar_dari' and bln='$bln' and thn='$thn'")->result();
										if ($data_buk_coa > 0) {
...
*/
								...
								echo "<td align='left'><a href='jurnal/detail_buk/" . $valR->nomor . "' data-toggle='tooltip' title='Click for detail'>" . $valR->nomor . "</a></td>";
