			...
			<form method="post" action="<?=base_url()?>index.php/order/proses_barang_use">
              ...
              <table class="table table-bordered">
				<tr class="widget-user-header bg-yellow">
					<th colspan="6" style="text-align:center;font-size:20px;" ><center>List Pengajuan Barang SPD</center></th>
				</tr>
				<tr>
					<th>No</th>
					<th>Nama Barang</th>
				...
				<?php
				if($list_barang_non_paket > 0){
					foreach($list_barang_non_paket as $rowx){
				?>
				<tr id="show_stock_<?=$rowx->id?>">
					<td colspan="6">
						<div class="form-group has-success col-lg-2">
							<input type="text" value="<?=$rowx->area?>">
							<select class="form-control" id="area" value ="" name="area[]">
								<?php
								if($list_area > 0){
									foreach($list_area as $rowsz){
									if($rowx->area == $rowsz->nm_area){
										$sel = "selected";
									}else{
										$sel = "";
									}
								?>
								<option value="<?=$rowsz->nm_area?>" <?=$sel?>><?=$rowsz->nm_area?></option>
								<?php
									}
								}
								?>
						...
						<select class="chosen-select form-control" name="nm_barang_2[]" id="nm_barang_2[]" data-placeholder="Choose a Location" tabindex="10">
								...
								if($list_kategori > 0){
									...
									foreach($list_kategori as $row){
										$i++;
										if($rowx->id_barang == $row->id){
											echo "<option selected value='".$row->id."+^".$row->nm_barang."'>".$row->nm_barang."</option>";
										}else{
						...
						<div class="form-group has-success col-lg-2">
						  <input type="text" class="form-control" id="qty" name='qty_2[]' placeholder="Masukan jumlah " value="<?=$rowx->qty?>">
						</div>
						<div class="form-group">
							<p class="btn btn-danger" onclick="return hapus(<?=$rowx->id?>)">Hapus</p>
						</div>
					</td>
				</tr>
				<?php
				...
				<tr>
					<td colspan="6">
						...
						<div class="form-group has-success col-lg-2">
						  <input type="text" class="form-control" id="qty" name='qty_2[]' placeholder="Masukan jumlah " value="">
						</div>
				...
				<tr>
					<td colspan="6" align="center"><input type="submit" class='btn btn-success btn-lg' name='submit_edit' value='Save' onclick="return check();"></td>
				</tr>
              </table>
			  </form>
            </div>
...
function hapus(id){
	var r = confirm("Hapus ?");
	if (r == true) {
		$.get( "<?= base_url(); ?>index.php/order/hapus_use" , { option : id } , function ( data ) {
