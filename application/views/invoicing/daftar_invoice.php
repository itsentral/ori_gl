    ...
    <div class="row">
        <div class="col-xs-12">
          <div class="box">
            ...
            <div class="box-body">
				<form action="<?=base_url()?>index.php/invoice/daftar_change" method="post">
				...
				<tbody>
					...
					<tr>
					  ...
					  <td><?=date("d-M-Y",strtotime($row->due_date))?></td>
						<td><?=$row->ditagihkan_oleh?></td>
					  <td><?=$row->billed_to?></td>
					  <td><?php if($row->bayar <= 0){echo "BELUM LUNAS";}else{echo "LUNAS";}?></td>
					...
					<?php
						...
						?>
								<a href="<?=base_url()?>index.php/invoice/payment/<?=$row->invoice_no?>" class="btn btn-warning btn-sm" width="20%" title='Payment'><i class="fa fa-money"></i></a>

								<a href="<?=base_url()?>index.php/invoice/edit_invoice/<?=$row->invoice_no?>/<?=$row->id_prospek?>/<?=$row->bayar_no?>" class="btn btn-primary btn-sm" width="20%" title='Edit Invoice'><i class="fa fa-edit"></i></a>

					  			<a href="<?=base_url()?>index.php/invoice/print_invoice2/<?=$row->id_prospek."/".$row->bayar_no?>" class="btn btn-success btn-sm" width="20%" title='Print Invoice' target="blank"><i class="fa fa-print"></i></a>
							
								<?php
								if($this->session->userdata('pn_jabatan')=="0" && $row->invoice_no !=''){//hanya admin yang bisa batal
									?>
									<a href="<?=base_url()?>index.php/invoice/cancel/<?=$row->invoice_no."/".$row->id_prospek?>" class="btn btn-danger btn-sm" width="20%" title='Batal Invoice'><i class="fa fa-close"></i></a>
									<?php
								}
							}
						}

						if($this->session->userdata('pn_jabatan')=="0" && $row->bayar > 0){//hanya admin yang bisa edit
						?>
						<?php
							$no_penawaran = str_replace("|","_",$row->id_penawaran);
							$nganten = str_replace(" & ","_",$row->billed_to);							
						?>
						<a href="<?=base_url()?>index.php/invoice/cancel_byr/<?=$row->invoice_no."/".$row->id_prospek."/".$nganten."/".$row->bayar."/".$no_penawaran."/".$row->bayar_via?>" class="btn btn-danger btn-sm" width="20%" title='Batal Bayar'><i class="fa fa-close"></i></a>
						<!--
							<a href="<?=base_url()?>index.php/invoice/payment/<?=$row->invoice_no?>" class="btn btn-info btn-sm" width="20%" title='Edit Payment'><i class="fa fa-edit"></i></a>
