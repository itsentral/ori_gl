...
class Jurnal_model extends CI_Model
{
	...
	{
		$query = "SELECT a.*,b.nama FROM jurnal a 
		left join coa_master b on a.no_perkiraan=b.no_perkiraan
		where a.nomor = '$nomor_jurnal' order by id";
		$query = $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	public function get_detail_bum($nomor_jurnal)
	{
		$query = "SELECT a.*,b.nama FROM jurnal a 
		left join coa_master b on a.no_perkiraan=b.no_perkiraan
		where a.nomor = '$nomor_jurnal' order by debet DESC";
		$query = $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	public function get_detail_buk($nomor_jurnal)
	{
		$query = "SELECT a.*,b.nama FROM jurnal a 
		left join coa_master b on a.no_perkiraan=b.no_perkiraan
		where a.nomor = '$nomor_jurnal' order by debet DESC";
	...
	public function get_detail_jurnal($tipe_jurnal, $nomor_jurnal)
	{
		...
		WHERE jurnal.tipe='$tipe_jurnal' and jurnal.nomor='$nomor_jurnal' AND (jurnal.debet <> 0 OR jurnal.kredit <> 0)
...
*/
	...
	public function get_noperkiraan($bln_aktif, $thn_aktif)
	{
		...
		$query 	= "SELECT * FROM COA WHERE level='5' and bln='$bln_aktif' and thn='$thn_aktif' and kdcab like '%$kode_cabang%' ORDER BY no_perkiraan ASC";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	public function list_dana_keluar($tanggal_awal, $tanggal_akhir)
	{
		...
		$query 	= "SELECT * from japh where  batal='0' and tgl between '$tanggal_awal' and '$tanggal_akhir' ORDER BY nomor DESC";//kdcab ='$kode_cabang' AND
	...
	{
		$query 	= "SELECT a.*, b.nama from jurnal a left join coa_master b on a.no_perkiraan=b.no_perkiraan where a.nomor='$nomorjurnal' and a.tipe='$tipe'";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	{
		$kode_cabang	= $this->session->userdata('kode_cabang');
		$query 	= "SELECT * FROM japh WHERE  tgl BETWEEN '$tanggal' AND '$tanggal2' AND batal ='0' ORDER BY tgl ASC";//kdcab = '$kode_cabang' AND
	...
	public function get_bank($bln_aktif, $thn_aktif)
	{
		...
		$query 	= "SELECT * FROM COA WHERE (no_perkiraan like '1101%' OR no_perkiraan like '1102%') AND level like '%5%' and kdcab like '%$kode_cabang%' and bln='$bln_aktif' and thn='$thn_aktif'";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	function inputDanaMasuk()
	{
		...
		$var_totalusd		= str_replace(',', '', $this->input->post('totalusd'));
		$Tgl_Jurnal		= date("Y-m-d", strtotime($this->input->post('tanggal_tampil')));
		...
		foreach ($det_Detail as $key => $vals) {
			...
			$nilai_debetusd		= str_replace(',', '', $vals['jumlahh']);
			...
			if($nilai_debet>0){
				...
				$Detail_BUM[$intL]		= array(
					...
					'kredit'        => $nilai_debet,
					'nilai_valas_debet'         => '0',
					'nilai_valas_kredit'        => $nilai_debetusd
				);
				...
				$Detail_BUM[$intL]		= array(
					...
					'created_by'	=> $this->session->userdata('pn_name'),
					'created_on'	=> date('Y-m-d H:i:s'),
					'debet'         => abs($nilai_debet),
					'kredit'        => '0',
					'nilai_valas_debet'        => $nilai_debetusd,
		...
		$Detail_BUM[$intL]		= array(
			...
			'debet'         => $var_total,
			'kredit'        => '0',
			'nilai_valas_debet'        => $var_totalusd,
			'nilai_valas_kredit'       => '0'
		...
		$Qry_Update_Cabang_acc	 = "UPDATE pastibisa_tb_cabang SET nobum=(nobum + 1) WHERE nocab='".$nomor_cabang."'";
...
*/
	...
	function inputDanaKeluar()
	{
		...
		$Detail_BUK[$intL]		= array(
			...
			'debet'         => '0',
			'kredit'        => $var_total,
			'nilai_valas_kredit'       =>  $var_totalusd,
			'nilai_valas_debet'        => '0'
		);
		$nomor_cabang	= $this->session->userdata('nomor_cabang');
		$Header_BUK		= array(
		...
		$Qry_Update_Cabang_acc	 = "UPDATE pastibisa_tb_cabang SET nobuk=(nobuk + 1) WHERE nocab='".$nomor_cabang."'";
		$this->db->query($Qry_Update_Cabang_acc);
/*
