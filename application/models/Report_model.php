...
class Report_model extends CI_Model
{
	...
	public function get_detail_jurnal2($nokir_induk, $var_tgl_awal, $var_tgl_akhir)
	{
		$kode_cabang = $this->session->userdata('kode_cabang');
		//$query 	= "SELECT * from jurnal where no_perkiraan between '$filter_nokir' and '$filter_nokir2' and tanggal between '$var_tgl_awal' and '$var_tgl_akhir' and nomor like '$kode_cabang%' order by id";
		$query 	= "SELECT * from jurnal where no_perkiraan = '$nokir_induk' and tanggal between '$var_tgl_awal' and '$var_tgl_akhir' order by tanggal asc";
	...
	{
		$kode_cabang	= $this->session->userdata('kode_cabang');
		$query 	= "SELECT * FROM COA WHERE (no_perkiraan like '41%') AND level='$level' AND bln='$var_bulan' AND thn='$var_tahun' and kdcab like '%$kode_cabang%' ORDER BY no_perkiraan ASC";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	{
		$kode_cabang	= $this->session->userdata('kode_cabang');
		$query 	= "SELECT * FROM COA WHERE (no_perkiraan like '51%') AND level='$level' AND bln='$var_bulan' AND thn='$var_tahun' and kdcab like '%$kode_cabang%' ORDER BY no_perkiraan ASC";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	{
		$kode_cabang	= $this->session->userdata('kode_cabang');
		$query 	= "SELECT * FROM COA WHERE (no_perkiraan like '52%') AND level='$level' AND bln='$var_bulan' AND thn='$var_tahun' and kdcab like '%$kode_cabang%' ORDER BY no_perkiraan ASC";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	public function get_Hutang22($var_bulan, $var_tahun, $level)
	{
		$kode_cabang	= $this->session->userdata('kode_cabang');
		$query 	= "SELECT * FROM COA WHERE (no_perkiraan like '22%') AND level='$level' AND bln='$var_bulan' AND thn='$var_tahun' and kdcab like '%$kode_cabang%' ORDER BY no_perkiraan ASC";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
...
//		$where  ="status='Aktif'";
		...
		if(!empty($where)){
			...
			//$this->db->distinct();
			$query = $this->db->get_where(DBACC.".supplier",$where);

		}else{
			//$this->db->distinct();
			$this->db->from(DBACC.".supplier");
//			$this->db->where("deleted", "0");
		...
		if($results){
			foreach($results as $key=>$vals){
				$aMenu[$vals['id_supplier']]	= $vals['nm_supplier'] .($vals['deleted']==1?' /'.$vals['id_supplier']. '(deleted)':'');
			}
		}
		return $aMenu;
	...
	public function GetWarehouseCOa(){
		$Temp_COA		= array();
		$Query_COA		= "SELECT coa_1 FROM warehouse WHERE NOT(coa_1 IS NULL OR coa_1 = '' OR coa_1 = '-') GROUP BY coa_1 ORDER BY coa_1 ASC";
		$rows_WH_COA	= $this->ori_operasional->query($Query_COA)->result();
		if($rows_WH_COA){
			foreach($rows_WH_COA as $keyCoa=>$valCoa){
				$Nomor_COA	= $valCoa->coa_1;
				$Temp_COA[$Nomor_COA]	= $Nomor_COA;
			}
		}
