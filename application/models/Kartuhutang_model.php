...
class Kartuhutang_model extends CI_Model
{
	...
	function GetData($awal,$vendor,$tipe=''){
		...
		FROM kartu_hutang WHERE tanggal < '$awal' AND id_supplier='$vendor' and no_perkiraan like '%".$tipe."%' order by tanggal ";
	...
	function GetDataAll($awal,$vendor,$tipe=''){
		$query 	= "SELECT sum(kredit - debet) as saldo
		FROM kartu_hutang WHERE tanggal < '$awal' AND no_perkiraan like '%".$tipe."%' order by tanggal ";
		$query	= $this->db->query($query);
		if ($query->num_rows() > 0) {
			return $query->result();
		} else {
	...
	public function get_detail_kartu_hutang($awal,$akhir,$vendor,$tipe='')
	{
		$query 	= "SELECT * from kartu_hutang WHERE id_supplier='$vendor' AND tanggal BETWEEN '$awal' AND '$akhir' and no_perkiraan like '%".$tipe."%' order by tanggal";
	...
	public function get_detail_kartu_hutang_all($awal,$akhir,$vendor,$tipe='')
	{
		$query 	= "SELECT * from kartu_hutang WHERE tanggal BETWEEN '$awal' AND '$akhir' and no_perkiraan like '%".$tipe."%' order by tanggal";
	...
	public function get_rekap_kartu_hutang($awal,$akhir,$vendor,$tipe)
	{
