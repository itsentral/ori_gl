...
foreach ($no_bum as $key => $nobum) {
    ...
    $jur_kasbank  = $this->m_list->getSQL("SELECT * FROM jurnal WHERE nomor = '" . $nobum . "' and (tipe = 'BUM' OR tipe LIKE'J%') and no_perkiraan like '1102%' or no_perkiraan like '1101%' order by nomor");
    ...
    $data_buk_coa1    = $this->m_list->getSQL("SELECT * FROM coa WHERE no_perkiraan = '$nokir_kasbank1' and bln='$bln' and thn='$thn' and kdcab='$kode_cabang'")->row();
...
?>
        ...
        <table class="gridtable" width="100%" style="border-spacing:0 -3px;">
            ...
            ?>
                ...
                $jumlah        = '';
                $jenis_tr    = '';
                if (substr($jarh_detail[$i]["no_perkiraan"], 0, 4) !== "1102" && substr($jarh_detail[$i]["no_perkiraan"], 0, 4) !== "1101") {
                    if ($i < $total_detail) {
                    ...
                    $data_buk_coa2    = $this->db->query("SELECT * FROM coa WHERE no_perkiraan = '$nokir_kasbank2[$i]' and kdcab='$kode_cabang'")->result();
                ...
                ?>
                    <tr>
                        ...
                        <td class="text-left" style="border-left:none;border-right:none;border-bottom:none;border-top:none; font-size:11px;"><?php echo substr($jarh_detail[$i]["keterangan"], 0, 53); ?></td>

                        <!-- <td class="text-left" style="border-left-style: dotted; border-right:none;border-bottom:none;border-top:none;font-size:10px;"><?php echo $jarh_detail[$i]["no_reff"]; ?></td> -->
                        ...
                        <td style="border-left-style: dotted; border-right:none;border-bottom:none;border-top:none;font-size:11px;"><?php echo $jarh_detail[$i]["no_perkiraan"]; ?></td>
        ...
        <div class="footer-text" style="font-size:11px;padding-top:-20px;padding-bottom:-2px;">Terbilang :
            <?php
            if ((int) $j === (int) $page) {
                echo substr(Terbilang($jarh->jml), 0, 80);
            } else {
