...
class Jurnal extends CI_Controller
{
	...
	public function list_jv_old()
	{
		...
		$data['bln_aktif']		= $bln_aktif;
		$data['thn_aktif']		= $thn_aktif;
	...
	function list_jv()	{
		...
		$this->load->view('jurnal/v_list_jv_new', $data);
	}
	public function get_data_json_list_jv_new(){
		$requestData	= $_REQUEST;
		$fetch			= $this->query_data_list_jv_new(
			$requestData['thn'],
			$requestData['bln'],
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData 	= array();
			$nestedData[]	= "<div align='left'><a href='".base_url()."index.php/jurnal/detail_jv/".$row['nomor']."' data-toggle='tooltip' title='Click for detail'>".$row['nomor']."</a></div>";
			$nestedData[]	= "<div align='left'>".date('d-M-Y', strtotime($row['tgl']))."</div>";
			$nestedData[]	= "<div align='left'>".($row['keterangan'])."</div>";
			$nestedData[]	= "<div align='left'>".($row['memo'])."</div>";
			$nestedData[]	= "<div align='right'>".number_format($row['jml'],2)."</div>";
			$nestedData[]	= "<div align='left'><a href='".base_url()."index.php/jurnal/edit_jv/".$row['nomor']."' data-toggle='tooltip' title='Edit Jurnal'>Edit Data</a></div>";
			$data[] = $nestedData;
		}
		$json_data = array(
	...
	public function query_data_list_jv_new($thn, $bln, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$where_text = "";
		if($bln <> '0'){
			$where_text.= " bulan='$bln' and tahun='$thn' ";
		}else{
			$where_text.= " tahun='$thn' ";
		}
		$sql = "
			SELECT * from javh a where  ".$where_text."
			AND (
				a.nomor LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.keterangan LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.memo LIKE '%".$this->db->escape_like_str($like_value)."%'
			)
		";
	...
	function edit_jv()
	{
		...
		$this->load->view("jurnal/edit_jv", $data);
	}
	
	public function save_edit_jv(){
		$post     = $this->input->post();
		$id       = $post['id'];
		$debet    = str_replace(".","",$post['debet']);
		$kredit   = str_replace(".","",$post['kredit']);
		$debetkurs    = $post['debetkurs'];
		$kreditkurs   = $post['kreditkurs'];  
		$coa   = $post['coa'];  
		...
		$this->db->trans_begin();
	 	...
	 	$this->db->query("UPDATE jurnal set no_perkiraan='$coa', debet='$debet', kredit='$kredit', nilai_valas_debet='$debetkurs', nilai_valas_kredit='$kreditkurs', modified_on=NOW() where id='$id'");
	...
	function detail_buk()
	{
		...
		$data['judul']		= "Detail BUK";
		$data['list_data'] 	= $this->Jurnal_model->get_detail_buk($nomor_jurnal);

		$this->load->view("jurnal/v_detail_buk", $data);
	}
...
//new 
	function list_dana_keluar()	{
		$this->load->view('jurnal/v_list_dana_keluar_new');
	}
	public function get_data_json_dana_keluar_new(){
		$requestData	= $_REQUEST;
		$fetch			= $this->query_data_dana_keluar_new(
			$requestData['tanggal'],
		...
		foreach($query->result_array() as $row)
		{
			...
			$nestedData[]	= "<div align='left'>".date('d-M-Y', strtotime($row['tgl']))."</div>";
			$nestedData[]	= "<div align='left'>".($row['jenis_reff'])."</div>";
			$namacoa = "";
			$data_uang_keluar_dari = $this->db->query("SELECT a.no_perkiraan, b.nama FROM jurnal a left join (SELECT no_perkiraan,nama FROM coa_master) b on a.no_perkiraan=b.no_perkiraan
			WHERE a.tipe='BUK' and a.nomor='".$row['nomor']."' and a.no_perkiraan like '11%'")->result();
			if ($data_uang_keluar_dari > 0) {
				foreach ($data_uang_keluar_dari as $row_jurnal1) {
					$namacoa=$row_jurnal1->no_perkiraan." ".$row_jurnal1->nama;
				}
			}
			$nestedData[]	= "<div align='left'>".$namacoa."</div>";
			$nestedData[]	= "<div align='left'>".($row['bayar_kepada'])."</div>";
			$nestedData[]	= "<div align='left'>".($row['note'])."</div>";
			$nestedData[]	= "<div align='right'>".number_format($row['jml'])."</div>";
			...
			<a href='".base_url()."index.php/jurnal/vmodal_batal_buk/".$row['nomor']."' title='Batal' onclick='return validasi_hapus()' class='btn btn-danger btn-sm'><i class='fa fa-close'></i></a></div>";
			$data[] = $nestedData;
		}
		$json_data = array(
			"draw"            	=> intval( $requestData['draw'] ),
	...
	public function query_data_dana_keluar_new($tanggal,  $tanggal2, $like_value = NULL, $column_order = NULL, $column_dir = NULL, $limit_start = NULL, $limit_length = NULL){
		$where_text = "";
		if($tanggal <> ''){
			$where_text.= " AND a.tgl>='".cdates($tanggal)."' ";
		}
		if($tanggal2 <> ''){
			$where_text.= " AND a.tgl<='".cdates($tanggal2)."' ";
		}
		$sql = "
			SELECT * from japh a where batal!='1' ".$where_text."
			AND (
				a.nomor LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.no_reff LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.customer LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.bayar_kepada LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.note LIKE '%".$this->db->escape_like_str($like_value)."%'
				OR a.jenis_reff LIKE '%".$this->db->escape_like_str($like_value)."%'
			)
		";
		$data['totalData'] = $this->db->query($sql)->num_rows();
		$data['totalFiltered'] = $this->db->query($sql)->num_rows();
