		...
		if ($this->input->post('posting') == "Posting") {
									...
									if ($cek_bt_ada > 0) { // jika bulan dan tahun yg dipilih ada di database
										...
										if ($cek_dk_nol > 0) {	// jika debet kreditnya lebih dari nol atau ada isinya, berarti belum melakukan unposting					
											...
											if ($data_coa_blnthn_terpilih > 0) {
												foreach ($data_coa_blnthn_terpilih as $row_post) {
													...
													if ($data_jurnal > 0) {
														foreach ($data_jurnal as $row01) {
															...
															$this->db->query("UPDATE COA set debet = '$sumDebit01', kredit = '$sumKredit01' WHERE no_perkiraan='$nokir' AND bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
														}
														...
														if ($data_coa_lv5 > 0) {
															...
															$this->db->query("UPDATE COA set debet = '$sumDebit5', kredit = '$sumKredit5' WHERE no_perkiraan like '$nokir_lv3%' AND level='3' AND bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
															// update debet kredit nokir lv4 di tabel coa
															$this->db->query("UPDATE COA set debet = '$sumDebit5', kredit = '$sumKredit5' WHERE no_perkiraan like '$nokir_lv4%' AND level='4' AND bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
														}
													}
												}
												...
												$this->db->query("UPDATE COA set saldoawal = '$laba' WHERE (no_perkiraan = '3903-01-01' or no_perkiraan = '3903-01-00' or no_perkiraan = '3903-00-00') AND bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
												...
												if ($var_bulan <> 1) {
													...
													$this->db->query("UPDATE COA set saldoawal = saldoawal+$laba_bulan_lalu_3903 WHERE (no_perkiraan = '3902-01-01' or no_perkiraan = '3902-01-00' or no_perkiraan = '3902-00-00') AND bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
			...
			if ($data_unpost > 0) {	// jika bulan dan tahun yg dipilih ada di database
				foreach ($data_unpost as $row_up) {
					...
					$this->db->query("UPDATE COA set debet = '0', kredit = '0' WHERE bln = '$var_bulan' AND thn = '$var_tahun' AND kdcab like '%$kode_cabang%'");
				}
				...
				if ($data_unpost2 > 0) {
					foreach ($data_unpost2 as $row_up2) {
						...
						$kode_cabang	= $this->session->userdata('kode_cabang');
						$newkodecabang=$this->Posting_model->get_pastibisa_by_nocab($kode_cabang);
					...
					if ($data_unpost3 > 0) {
						foreach ($data_unpost3 as $row_up3) {
							...
							$newkodecabang=$this->Posting_model->get_pastibisa_by_nocab($kode_cabang3);
							$kode_cabang3=$newkodecabang->nocab.'-'.$newkodecabang->subcab;
							//	copy seluruh nomor perkiraan dalam tabel coa, buat baru dengan bulan dan tahun yg dipilih
