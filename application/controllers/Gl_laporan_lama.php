...
class Gl_laporan extends CI_Controller
{
	...
	function master_coa()	// edit by rin 171219
	{
		...
		$cek_coamaster	= $this->Model_latihan->get_coa_master2(); // mengecek, sudah ada apa belum, coa utk cabang ini
		if ($cek_coamaster > 0) { // jika sudah ada
			...
			$cek_coa	= $this->Model_latihan->get_coa($bln_periode, $thn_periode); // ambil data coa dari tabel coa
			if ($cek_coa > 0) {
	...
	public function delete_master_supplier($id)
	{ //dellete supplier
		$id = $this->uri->segment(3);
	...
	function proses_add_master_supplier()
	{ //proses add supplier
