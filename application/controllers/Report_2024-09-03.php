		...
		if($this->input->post()){
			...
			if($rows_WHR){
				...
				if($Tgl_Stock < date('Y-m-d')){
					$Query_Material		= "SELECT * FROM warehouse_stock_per_day WHERE id_gudang IN('".str_replace(",","','",$Code_WHR)."') AND qty_stock <> 0 AND hist_date LIKE '".$Tgl_Stock."%' ORDER BY nm_material ASC";
					$rows_Material		= $this->ori_operasional->query($Query_Material)->result();
					...
					*/
					...
					$Query_Material		= "SELECT * FROM warehouse_stock WHERE id_gudang IN('".str_replace(",","','",$Code_WHR)."') AND qty_stock <> 0 ORDER BY nm_material ASC";
					$rows_Material		= $this->ori_operasional->query($Query_Material)->result();
